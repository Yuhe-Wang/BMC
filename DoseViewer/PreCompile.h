...
#include "../DcmTK/DcmTK.h" // dicom support
...
const wxString MODULENAME = "DoseViewer";
...
struct FloatRect //aimed to resolve continuous resizing problem with wxRect
{
	...
	bool within(float xin, float yin)
	{
		xin -= x;
		yin -= y;
		if (xin<0 || xin>width || yin<0 || yin>height) return false;
		else return true;
	}
	void setEmpty()
	{
...
enum ViewDirection
{
	ZAXIS = 0 , YAXIS, XAXIS, VIEW3D //this must have the same order shown in the choice
};

enum FIGURE_TYPE
{
	FIGURE_2D = 0 , FIGURE_3D
};
...
//used to import the CT image set
class SliceInfo
{
public:
	double * density;
...
enum VIEW3D_EVENT //possible event View3D class may emitt.
{
	...
	VIEW3D_CUBOID_SAMPLING_FINISHED, // the cuboid sampling has been finished
	VIEW3D_CUBOID_SAMPLING_CHANGED
};



//declaration of some C-style function
int searchViewRayCouch(ArrayMgr<SFloat>& den);
#endif

	...
	if (mat != NULL)
	{
		...
		if (b_globalISOLines)
		{
			...
			else
			{
				...
				if (fabs(maxv - minv - nc*dz) < 1e-9) --nc;
				if (nc == 0)
				{
	...
	//draw the legend
	{
		...
		for (int i = 0; i < nl; ++i)
		{
			...
			if (b_globalISOLines)
			{
				...
				//sprintf(ch, "%.4g Gy", percent*stdValue);
				sprintf(ch, "%.3g%%", percent * 100);
			}
			else
			{
				//sprintf(ch, "%.4g", percent*pv[wid].slice.Max());
				//sprintf(ch, "%.4g Gy", percent*pv[wid].slice.Max());
				sprintf(ch, "%.3g%%", percent*100);
			}
...
SFloat View3D::getPointValue(SFloat x, SFloat y, SFloat z)
{
	...
	ArrayMgr<SFloat>* pmat = mat;
	if (pmat == NULL) pmat = bgImage;
	if (pmat == NULL) return -1; // no data to sample
	...
	SFloat y1 = 0, y2 = 0;
	y1 = pmat->a(ix, iy, iz)*(1 - x) + pmat->a(ix + 1, iy, iz)*x;
	y2 = pmat->a(ix, iy + 1, iz)*(1 - x) + pmat->a(ix + 1, iy + 1, iz)*x;
	SFloat z1 = y1*(1 - y) + y2*y;

	y1 = pmat->a(ix, iy, iz + 1)*(1 - x) + pmat->a(ix + 1, iy, iz + 1)*x;
	y2 = pmat->a(ix, iy + 1, iz + 1)*(1 - x) + pmat->a(ix + 1, iy + 1, iz + 1)*x;

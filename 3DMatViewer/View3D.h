...
struct MarkPoint
{
	MarkPoint(double xi, double yi, double zi, bool shown = true, float radiusi = 0.5, uint8_t r = 255, uint8_t g = 0, uint8_t b = 0)
	{
		x = xi;
		y = yi;
		z = zi;
	...
	double x, y, z; // it's relative to the patient coordinates
	bool b_show;
...
public:
	...
	bool initShow();// only call after all parameters has been set
	
	
	void render(); // only do the rendering job without doing any necessary adjustment. Used in OnPaint() function
	void show();   // do necessary recalculation and then call render()
	...
	//interface
	void showBox(unsigned int b = 2){ if (b > 1) b_showBox = !b_showBox; else b_showBox = b; render(); }//change whether to draw the box
	void showMat(unsigned int b = 2){ if (b > 1) b_showMat = !b_showMat; else b_showMat = b; render(); }//change whether to show the mat
	void showGlobalContrast(unsigned int b = 2){ if (b > 1) b_globalContrast = !b_globalContrast; else b_globalContrast = b; show(); }
	void showContours(unsigned int b = 2){  }
	...
	void setCuboidOrigin(double copx, double copy, double copz)
	{
		COPX = copx;
		COPY = copy;
		COPZ = copz;
	...
	void showMarkPoint(unsigned int b = 2){ if (b > 1) b_showMarkPoint = !b_showMarkPoint; else b_showMarkPoint = b; render(); }
	...
	void getLineData(ArrayMgr<double>& xin, ArrayMgr<double>& yin)
	{
	...
	FigureFrame* newGraphFrame(bool b_create = true);//if b_create = false, it'll try to use existed graph

	/*data*/
protected:
	int NX, NY, NZ; // must be specified from external matrix
	double DX, DY, DZ; // default 0.1 cm
	double COPX, COPY, COPZ; // default (-LX/2, -LY/2, -LZ/2), position of phantom cuboid origin in patient coordinates.

...
int SourceHead_Sample(const void* rng, const void* pars)
{
	return gSH->sample((PRNG*)rng, (Particle*)pars);
}
...
void HeadTransport::transportPhoton(PRNG &rng, int aSegmentIndex, double E, Vector &x, Vector &u, int weight, int type,
	...
	if (weight > 1 && isInFOI) {
		...
		while (1) {
			...
			//
			if (nstep > 1000) {
				ZeusLog("Warning: HeadTransport::transportPhoton: more than 1000 steps taken through geometry\n");
				return;
			}
	...
	while (1) {
		...
		while (1) {
			...
			if (nstep > 1000) {
				ZeusLog("Warning: HeadTransport::transportPhoton(a): more than 1000 steps taken through geometry\n");
				return;
			}

...
#ifdef SOURCE_ENERGY_SORT
class ThreadSource // one thread to generate particles
{
public:
	ThreadSource(){}
	void init(int tid, int Capacity, int NSampleStack, Phantom* pph)
	{
		_Capacity = Capacity;
		_pph = pph;
		_rng.init(1234 + tid);
		...
		_PTemp = new Particle[NSampleStack];

		_np = new int[_NE];
		//create NE pool for different energy level
		_PPool = new ParticleR*[_NE]; //create the pointers of NE pools
		for (int i = 0; i < _NE; ++i)
		{
			_PPool[i] = new ParticleR[Capacity + NSampleStack];
			_np[i] = 0;
		}
	}
	~ThreadSource()
	{
		delete[] _PTemp;
		delete[] _np;
		for (int i = 0; i < _NE; ++i) delete[] _PPool[i];
		delete[] _PPool;
	}
	void generate() // generate particles, and report which energy grid is full
	{
		_hist = 0;
		while (true)
		{
			//check if any of the pool is full now
			for (int ie = 0; ie < _NE; ++ie)
			{
				if (_np[ie] >= _Capacity)
				{
					_idE = ie;
					return; // Don't forget to move the residual particles after copying the pool to GPU
				}
			}

			int nadd = SourceHead_Sample(&_rng, _PTemp);
			for (int i = 0; i < nadd; ++i)
			{
				if (_pph->lineInPhantom(_PTemp[i])) //add only if the particle goes into the phantom
				{
					//determine which energy grid it lies in
					double E = _PTemp[i].E;
					int ie = 0;
					if (fabs(E - 1.3325e6) < 1e2) ie = 1;
					else if (fabs(E - 1.1732e6) < 1e2) ie = 2;
					//else continue; // for performance analysis purpose
					ParticleR *p = _PPool[ie] + _np[ie];

					p->E = (ZFloat)E;
					p->u = (ZFloat)_PTemp[i].u;
					p->v = (ZFloat)_PTemp[i].v;
					p->w = (ZFloat)_PTemp[i].w;
					p->x = (ZFloat)_PTemp[i].x;
					p->y = (ZFloat)_PTemp[i].y;
					p->z = (ZFloat)_PTemp[i].z;
					p->type = photon;
					p->weight = (ZFloat)_PTemp[i].weight;
					++_np[ie];
				}
			}
			++_hist;	
	...
	{
		int ie = _idE;
		int rest = _np[ie] - _Capacity;//rest particles left in the buffer part
		for (int j = 0; j < rest; ++j) //need move the buffered particles to the left side
			_PPool[ie][j] = _PPool[ie][j + _Capacity];
		_np[ie] = rest;
	}

	int _Capacity; // how many particle to buffer
	int _NE; // how many energy intervals to divide.
	
	int _hist; // history has been sampled
	int _idE; // the chosen energy index of the pool
	ParticleR** _PPool; 
	Particle* _PTemp;
	int* _np; // how many particles is held currently
	PRNG _rng;
	Phantom* _pph;
};
...
{
public:
	SourcePool(vector<GPUConfig>* gcp,int NT, int NOneBatch, Phantom* pph, int NSampleStack = 400)
	{
		_gcp = gcp;
		_b_initBuff = true;
		...
		_ts = new ThreadSource[NT];
		for (int it = 0; it < NT; ++it) _ts[it].init(it, _PoolCapacity, NSampleStack, pph);
	}
	~SourcePool()
	{
		delete[] _ts;
...
#pragma omp parallel num_threads(_NT)
		...
		int sum = 0;
		for (int it = 0; it < _NT; ++it) sum += _ts[it]._hist;
...
#pragma omp parallel num_threads(_NT)
	...
	void copy2GPU(int ig) //should be called by each GPU thread
	{
		...
		{
			int ie = _ts[i]._idE;
			cudaMemcpyAsync(pp + i*_PoolCapacity, _ts[i]._PPool[ie], _PoolCapacity * sizeof(ParticleR), cudaMemcpyHostToDevice, gc[ig].copystream);
		}
...
public:
	SourcePool(vector<GPUConfig>* gcp, int NT, int NOneBatch, Phantom* pph, int NSampleStack = 400)
	{
		...
		for (int i = 0; i < NT; ++i)
		{
			cudaMallocHost(&_PPool[i], sizeof(ParticleR)*(_PoolCapacity + NSampleStack));
...
#pragma omp parallel num_threads(_NGenerateThread)
#endif
		{
			...
			while (_np[it] < _PoolCapacity)
			{
				...
				{
					ParticleR* p = pbase + j;
					//test if _PPoolTemp[it][i] will line in the phantom
					if (_pph->lineInPhantom(_PPoolTemp[it][i]))
					{
...
private:
	...
	bool _b_initBuff; //true means pointing to InitPars_A, false means pointing to InitPars_B
	vector<GPUConfig>* _gcp; //pointer to GPU config
	Phantom* _pph;// pointer to the phantom
	bool _b_clear;
};
#endif
...
public: \
	...
	{ \
		...
		cudaErrorCheck(cudaMemcpyToArray(cuArray, 0, 0, p, width * height * sizeof(ZFloat), cudaMemcpyHostToDevice)); \
...
__device__ __forceinline__ ZFloat get##name(float i, float j) \
{ \
	return tex2D(name##TextureData, i , j); \
} 
/*************************End: type/class definition *********************************/

void executeJob(const char* configFileName, double processWeight, MPS& configMacro);
#endif
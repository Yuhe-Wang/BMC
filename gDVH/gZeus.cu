...
__constant__ int ForwardDetect = 1; //whether do forward advancing movement when guiding in the photon
...
__constant__ SFloat* orgid; //pointer to phantom
...
{
	//judge if the electron's range can enter the important region
	if (orgid)
	{
		ZFloat range = (p.E - ERangeCut)*WaterRangeCS(p.E);
		if (range < orgid[p.iabsv]*DX) //deposit energy in current voxel and return
		{
			deposit(p.iabsv, p.E*p.weight);
			return;
		}
...
void GZEUS::phantom2GPU() //copy phantom info to GPU. Make sure the phantom has loaded data
{
	...
	for (unsigned int i = 0; i < gc.size(); ++i)
	{
		...
		if (_phant->orgid.getInnerLength() > 0)
		{
			cudaErrorCheck(cudaMalloc(&gc[i].d_orgid, NVoxel * sizeof(SFloat)), "resize orgid");
			cudaErrorCheck(cudaMemcpyToSymbol(orgid, &gc[i].d_orgid, sizeof(SFloat *)), "copy orgid pointer to GPU constant"); //set the array pointer
			cudaErrorCheck(cudaMemcpy(gc[i].d_orgid, _phant->orgid.getP(), sizeof(SFloat)*NVoxel, cudaMemcpyHostToDevice), "init the value of orgid in GPU");
		}
		else
		{
			gc[i].d_orgid = NULL;
...
void GZEUS::freeGPU()//do some clean up
{
	for (unsigned int i = 0; i < gc.size(); ++i)
	{
		...
		cudaErrorCheck(cudaFree(gc[i].d_orgid));
		gc[i].destroyStream();
	}
}

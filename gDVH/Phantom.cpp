	...
	if (b_fromFile)
	{
		...
		if (BF.read(fname.c_str()))//try to load infomation from *.phtm file
		{
			...
			else //need to convert to relative density first
			{
				...
				int LEN = NX*NY*NZ;
				for (int i = 0; i < LEN; ++i) ph.a(i) = SFloat(ph.a(i)/penMat.density(matid.a(i)));
			}
		}
		else //try to load RED file then
		{
			...
			string importantRegion;
			cf->getValue("importantRegion", importantRegion);
			if (importantRegion.size() > 0) //this option is effective
			{
				//load the label volume file
				//first check if PlanOverview.txt file exits
				fs::path fpath(fname);
				fs::path stemName = fpath.stem();

				fs::path batchFolder = fpath.parent_path();
				fs::path planOverview = batchFolder / "PlanOverview.txt";
				fs::path labelName = batchFolder / stemName;
				labelName += ".lvol";
				if (fs::exists(planOverview) && fs::exists(labelName))
				{
					//should first parse the PlanOverview.txt to get the contour names
					FILE *fp = fopen(planOverview.string().c_str(), "r");
					if (NULL == fp)
					{
						printf("Cannot open %s. load PlanOverview.txt error!\nPress enter to continue...\n", planOverview.string().c_str());
						getchar();
						return;
					}
					string vrPlan;
					...
					//find the contour names
					spos = vrPlan.find("Contours");
					spos = vrPlan.find("\n", spos) + 1;
					string contourName;
					...
					{
						epos = vrPlan.find("\n", spos);
						contourName = vrPlan.substr(spos, epos - spos); //get one line
						if (contourName.empty()) break; //empty line

						contourName = contourName.substr(0, contourName.find(":")); //extract the contour name
						if (contourName[0] == ' ') contourName.erase(0, contourName.find_first_not_of(' ')); //left trim
						if (contourName.back() == ' ') contourName.erase(contourName.find_last_not_of(' ') + 1); //right trim
						contours.push_back(contourName);
						spos = epos + 1;
					}
					int NContours = int(contours.size());

					//set the mask based on contour names and the definition in ImportantRegion.txt
					//If ImportantRegion.txt doesn't exist, the default region is PTV only.
					std::bitset<32> bitMask;
					string line;
					ifstream idfile(importantRegion);
					if (idfile.good())
					{
						while (getline(idfile, line))  // same as: while (getline( myfile, line ).good())
						{
							if (line[0] == ' ') line.erase(0, line.find_first_not_of(' ')); //left trim
							if (line.back() == ' ') line.erase(line.find_last_not_of(' ') + 1); //right trim
																								//compare line to contour names, and determine the bit mask
							for (int i = 0; i < NContours; ++i)
							{
								if (line == contours[i])
								{
									bitMask.set(i, 1);
									Log("the contour: %s is marked as important region\n", contours[i].c_str());
								}
							}
						}
						idfile.close();
					...
					{
						printf("\nCannot open the important region definition file. Will use PTV only now.\n\n");
						for (int i = 0; i < NContours; ++i)
						{
							if (contours[i] == "PTV")
							{
					...
					// get the skin bit mask. It will be used to determine if a voxel is inside the skin
					for (int i = 0; i < NContours; ++i)
					{
						if (contours[i] == "Skin")
						{
					...
					//initialize the orgid matrix with lvol file and 
					fp = fopen(labelName.string().c_str(), "rb");
					if (NULL == fp) return;
					uint32_t temp32;
					double temp64;
					fread(&temp32, sizeof(uint32_t), 1, fp); //Marjor_Header
					fread(&temp32, sizeof(uint32_t), 1, fp); //Minor_Header
					fread(&temp32, sizeof(uint32_t), 1, fp);
					fread(&temp64, sizeof(double), 1, fp);
					fread(&temp64, sizeof(double), 1, fp);
					fread(&temp32, sizeof(uint32_t), 1, fp);
					fread(&temp64, sizeof(double), 1, fp);
					fread(&temp64, sizeof(double), 1, fp);
					...
					for (int i = 0; i < len; ++i) lvol.a(i) = data[i];

					orgid.resize(NX, NY, NZ);
					for (int i = 0; i < len; ++i)
					{
						std::bitset<32> labelBit(data[i]);
						labelBit &= bitMask;
						if (labelBit.any()) orgid.a(i) = -1;
						else orgid.a(i) = 10e9; // set an extreme long distance
						// mark all the low density region inside the body as important region
						// if (ph.a(i) < 0.5) orgid.a(i) = -1;
					}

					delete[] data;					

					//output orgid for debug purpose
// 					VF.m = orgid;
// 					VF.m += 1;
// 					fs::path orgid_debug = batchFolder / "orgid_debug.red";
// 					VF.write(orgid_debug.string().c_str());

					setOrgID();

					// orgid = 10e9; // This just turn off eletron simulation
				}
			}
...
void Phantom::setOrgID() //this is a CPU implementation of calculating orgid; need to compare to GPU's performance
{
	for (int ix = 0; ix < NX; ++ix)
		for (int iy = 0; iy < NY; ++iy)
			for (int iz = 0; iz < NZ; ++iz)
			{
				if (orgid.a(ix, iy, iz) == -1) //this voxel belongs to an important region
				{
					RunTimeCounter t;
					recursiveWalk(ix, iy, iz, ix, iy, iz, 0, 0);
					if (t.stop() > 2e-3) Log(">2ms\n");
				}
			}
	// convert the distance to energy, maybe helpful
	// for (int i = 0; i < len; ++i)
...
// 	return ave_err;
// }
// bool Phantom::winthin(int ix, int iy, int iz)
// {
// 	return 0 <= ix&&ix < NX && 0 <= iy&&iy < NY && 0 <= iz&&iz < NZ;
// }

int maxLayer = 0; //debug purpose
void Phantom::recursiveWalk(int& cx, int& cy, int& cz, int ix, int iy, int iz, int layer, SFloat walked)
{
	if (layer > maxLayer)
	{
		Log("maxLayer increased to %d", layer);
		maxLayer = layer;
	}

	// compared the walked.
	SFloat rwalked = orgid.a(ix, iy, iz);
	if (rwalked != -1)
	{
		orgid.a(ix, iy, iz) = min(rwalked, walked);
		if (walked > 2) return; // cannot escape current voxel, so no need to do further test 
		walked += ph.a(ix, iy, iz);
	}
	...
	for (int sx = -1; sx <= 1; ++sx)
		for (int sy = -1; sy <= 1; ++sy)
			for (int sz = -1; sz <= 1; ++sz)
			{
				...
				int ay = iy + sy;
				int az = iz + sz;
				if (max(abs(ax - cx), max(abs(ay - cy), abs(az - cz))) == layer // in the outer layer
					&& 0 <= ax&&ax < NX && 0 <= ay&&ay < NY && 0 <= az&&az < NZ //within the voxel
					&& orgid.a(ax, ay, az) != -1 // it's not a IR voxel
					&& inSkin(ax, ay, az) // It's inside the skin. It will extremely time consuming after entering the air
					)
					recursiveWalk(cx, cy, cz, ax, ay, az, layer, walked);
			}
}
...
bool Phantom::inSkin(int ix, int iy, int iz)
{
	std::bitset<32> labelBit(lvol.a(ix, iy, iz));
	labelBit &= skinBitMask;
	return labelBit.any();
}
...
{
public:
	SourcePool(vector<GPUConfig>* gcp, int NT, int NOneBatch, octree_node* root, int NSampleStack = 400)
	{
		_gcp = gcp;
		_root = root;
		_b_initBuff = true;
...
#endif
		{
			...
			while (_np[it] < _PoolCapacity)
			{
				...
				for (int i = 0; i < nadd; ++i)
				{
					...
					int idt = _root->lineIn(_PPoolTemp[it][i].x, _PPoolTemp[it][i].y, _PPoolTemp[it][i].z, _PPoolTemp[it][i].u, _PPoolTemp[it][i].v, _PPoolTemp[it][i].w);
					if (idt != -1)
					{
